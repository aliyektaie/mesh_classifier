...
public class BinaryBuffer {
    ...
    /**
     * Seek by a similarity int the buffer.
     *
     * @param distance
     *            The similarity to be added to read index.
     * @throws Exception
     *             if the seek similarity is invalid.
     */
    public void seek(int distance) {
        if ((readIndex + distance >= beginIndex) && (readIndex + distance <= endIndex)) {
            readIndex += distance;
        } else {
            throw new RuntimeException("Invalid seek similarity.");
        }
    }
