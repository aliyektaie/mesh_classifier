...
// -------------------------- OTHER METHODS --------------------------
    ...
    public byte[] serialize() {
        int length = this.indexes.length * 4 + this.values.length * 4;
        length += 8;

        BinaryBuffer buffer = new BinaryBuffer(length);

        buffer.append(indexes.length);
        for (int i = 0; i < indexes.length; i++) {
    ...
    public static SparseVector load(byte[] data) {
        BinaryBuffer buffer = new BinaryBuffer(data);
        SparseVector result = new SparseVector();

        result.indexes = new int[buffer.readInt()];
        for (int i = 0; i < result.indexes.length; i++) {
            result.indexes[i] = buffer.readInt();
        }

        result.values = new float[buffer.readInt()];
        for (int i = 0; i < result.values.length; i++) {
            result.values[i] = buffer.readFloat();
        }
