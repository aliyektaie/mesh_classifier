...
/*	public BinaryModel(Properties props)
    ...
    public BinaryModel(Properties props, LabelParser<L> labelParser) {
        ...
        try {
            ...
            String kernelType = props.getProperty("kernel_type");
            if (kernelType.equals("linear")) {
                builder.kernel= (KernelFunction<P>) new LinearKernel();
            } else {
                builder.kernel =
                        (KernelFunction) Class.forName(kernelType).getConstructor(Properties.class)
                                .newInstance(props);
            }
        } catch (Throwable e) {
            throw new SvmException(e);
